    ...
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm font-sans opacity-50 uppercase tracking-widest dark:text-white">{course.university}</p>
        </div>
      </div>
      ...
      {!analysis && !isAnalyzing ? (
        <div className="max-w-4xl mx-auto space-y-8 py-12">
          <div className="text-center space-y-4">
            <h2 className="text-4xl font-bold dark:text-white">Ready to Predict?</h2>
            <p className="text-[#1a1a1a]/60 dark:text-white/60 font-sans">Upload your materials and let our AI find the patterns.</p>
          </div>
          
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <h3 className="text-sm font-sans font-bold uppercase tracking-widest opacity-50 dark:text-white/50 px-2">Lecture Resources</h3>
              <FileUpload 
                files={lectureFiles} 
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-sans font-bold uppercase tracking-widest opacity-50 dark:text-white/50 px-2">Assessment Resources</h3>
              <FileUpload 
          ...
          <button 
            ...
            className="w-full bg-[#1a1a1a] dark:bg-[#A8A878] text-white py-5 rounded-[2rem] font-sans font-bold text-lg flex items-center justify-center gap-3 hover:bg-[#333] dark:hover:bg-[#8A8A58] transition-all shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="max-w-2xl mx-auto py-24 text-center space-y-8">
          <motion.div 
            ...
            className="w-24 h-24 bg-[#5A5A40]/10 dark:bg-[#A8A878]/10 rounded-[2rem] flex items-center justify-center mx-auto text-[#5A5A40] dark:text-[#A8A878]"
          >
            <Sparkles className="w-12 h-12" />
          </motion.div>
          <div className="space-y-2">
            <h2 className="text-3xl font-bold dark:text-white">Analyzing Patterns...</h2>
            <p className="text-[#1a1a1a]/60 dark:text-white/60 font-sans">Our AI is scanning past papers and lecture notes.</p>
          </div>
          <div className="max-w-xs mx-auto space-y-4">
            ...
            <div className="flex justify-between text-[10px] font-sans font-bold uppercase tracking-widest opacity-40 dark:text-white/40">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="bg-white dark:bg-[#1C1C1C] rounded-[2rem] p-4 border border-black/5 dark:border-white/5 shadow-sm space-y-2">
              <TabButton 
            ...
            <div className="bg-[#1a1a1a] dark:bg-[#5A5A40] text-white rounded-[2rem] p-8 space-y-6">
              <div className="space-y-2">
                <h3 className="text-xl font-bold">Share Predictions</h3>
                <p className="text-white/60 text-sm font-sans">Help your classmates study smarter too.</p>
              </div>
              <div className="flex gap-3">
                <button 
                  onClick={sharePrediction}
                  className="flex-1 bg-white/10 hover:bg-white/20 p-4 rounded-2xl flex items-center justify-center gap-2 transition-all"
                >
                  <Share2 className="w-5 h-5" />
                  <span className="text-sm font-sans font-bold">Share</span>
                </button>
          ...
          <div className="lg:col-span-2 space-y-8">
            <AnimatePresence mode="wait">
              {activeTab === 'overview' && (
                ...
                >
                  {/* Heatmap Chart */}
                  <div className="bg-white dark:bg-[#1C1C1C] rounded-[2.5rem] p-10 border border-black/5 dark:border-white/5 shadow-sm space-y-8">
                    <div className="flex justify-between items-center">
                      <h3 className="text-2xl font-bold dark:text-white">Exam Probability Heatmap</h3>
                      <div className="flex items-center gap-2 text-xs font-sans opacity-50 dark:text-white">
                    ...
                    <div className="h-[300px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={analysis?.topics} layout="vertical" margin={{ left: 40 }}>
                          ...
                          <YAxis 
                            ...
                            type="category" 
                            width={120} 
                            tick={{ fontSize: 12, fontFamily: 'sans-serif', fontWeight: 600, fill: 'currentColor' }}
                  ...
                  <div className="grid gap-4">
                    {analysis?.topics.map((topic, idx) => (
                      <div key={idx} className="bg-white dark:bg-[#1C1C1C] p-6 rounded-3xl border border-black/5 dark:border-white/5 flex items-start gap-6">
                        <div className={`
                        ...
                        `}>
                          <span className="text-xl font-bold leading-none">{topic.probability}%</span>
                          <span className="text-[8px] font-sans font-bold uppercase tracking-widest mt-1">Chance</span>
                        </div>
                        <div className="space-y-1">
                          <h4 className="text-lg font-bold dark:text-white">{topic.name}</h4>
                          <p className="text-sm font-sans text-[#1a1a1a]/60 dark:text-white/60 leading-relaxed">{topic.reasoning}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* Repeated Questions */}
                  <div className="bg-white dark:bg-[#1C1C1C] rounded-[2.5rem] p-10 border border-black/5 dark:border-white/5 shadow-sm space-y-6">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-2xl font-bold dark:text-white">Repeated Question Detector</h3>
                    </div>
                    <div className="space-y-4">
                      {analysis?.repeatedQuestions.map((rq, idx) => (
                        <div key={idx} className="p-4 rounded-2xl bg-[#F5F5F0] dark:bg-white/5 border border-black/5 dark:border-white/5 flex justify-between items-center">
                          <span className="font-sans text-sm font-medium italic dark:text-white/80">"{rq.question}"</span>
                          <span className="text-[10px] font-sans font-bold bg-white dark:bg-[#2A2A2A] dark:text-white px-3 py-1 rounded-full shadow-sm border border-black/5 dark:border-white/5">
              ...
              {activeTab === 'questions' && (
                ...
                >
                  <div className="space-y-2">
                    <h3 className="text-3xl font-bold dark:text-white">Predicted Exam Questions</h3>
                    <p className="text-[#1a1a1a]/60 dark:text-white/60 font-sans">Realistic questions generated by AI based on your materials.</p>
                  </div>
                  <div className="grid gap-6">
                    {questions.map((q, idx) => (
                      <div key={idx} className="bg-white dark:bg-[#1C1C1C] p-8 rounded-[2rem] border border-black/5 dark:border-white/5 shadow-sm space-y-4">
                        <div className="flex justify-between items-start">
                          <span className="text-xs font-sans font-bold text-[#5A5A40] dark:text-[#A8A878] uppercase tracking-widest bg-[#5A5A40]/5 dark:bg-[#A8A878]/5 px-3 py-1 rounded-full">
                            Question {idx + 1}
                          </span>
                        </div>
                        <h4 className="text-xl font-bold leading-tight dark:text-white">"{q.question}"</h4>
                        <div className="pt-4 border-t border-black/5 dark:border-white/5">
                          <p className="text-sm font-sans text-[#1a1a1a]/60 dark:text-white/60 italic">
              ...
              {activeTab === 'priority' && (
                ...
                >
                  <div className="bg-white dark:bg-[#1C1C1C] rounded-[2.5rem] p-10 border border-black/5 dark:border-white/5 shadow-sm space-y-8">
                    <div className="space-y-2">
                      <h3 className="text-3xl font-bold dark:text-white">Study Priority Engine</h3>
                      <p className="text-[#1a1a1a]/60 dark:text-white/60 font-sans">A ranked study plan optimized for your exam.</p>
                    </div>
                    <div className="space-y-4">
                      {analysis?.studyPriority.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-6 group">
                          <div className="w-12 h-12 rounded-2xl bg-[#F5F5F0] dark:bg-white/5 group-hover:bg-[#5A5A40] dark:group-hover:bg-[#A8A878] group-hover:text-white transition-all flex items-center justify-center font-bold text-xl dark:text-white">
                            {idx + 1}
                          </div>
                          <div className="flex-1 p-5 rounded-2xl border border-black/5 dark:border-white/5 group-hover:border-[#5A5A40]/30 dark:group-hover:border-[#A8A878]/30 transition-all font-bold text-lg dark:text-white">
                            {item}
                          </div>
                        </div>
                      ))}
...
function TabButton({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) {
  return (
    <button 
      onClick={onClick}
      className={`
        w-full flex items-center gap-3 p-4 rounded-2xl transition-all font-sans font-bold text-sm
